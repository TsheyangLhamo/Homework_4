...
public class Problem_4 : MonoBehaviour
{
    ...
    void Start()
    {
        Transform v = GetComponent<Transform>();
        PrintOutNameAndPosition(v.name, v.position);
        Transform t= transform.GetChild(0).GetComponent<Transform>();
        PrintOutNameAndPosition(t.name, t.position);
        Transform z = GameObject.Find("Part_4_GameObject").GetComponent<Transform>();
        PrintOutNameAndPosition(z.name, z.position);
    }
